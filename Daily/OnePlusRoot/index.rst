...
.. note:: 
  ...
  求人不然求己，我之后用其他手机又试了一下，即使 KMI 版本完全一致，启动后还始终是显示
  ``未安装``。还得是自己直接找到手机对应版本的全量包提取出 ``init_boot.img`` （内核版本大于等于 5.15）后直接用 SukiSU 管理器修补后，再 ``fastboot flash init_boot`` 刷进去。

  内核版本小于 5.15 的话则是用的 ``boot.img``。
...
这个给是 `老LSPosed <https://github.com/LSPosed/LSPosed>`_  停止维护后的 `新LSPosed <https://github.com/JingMatrix/LSPosed>`_
...
OTA 下载安装后不要立即重启，而是先打开 SukiSU 在 ``安装`` 中选择 ``安装到另一个槽位``，然后再重启手机即可。

具体参考 `SukiSU 官方文档 <https://sukisu.org/guide/installation#maintaining-root-after-ota-updates>`_ 。
...
- 2025.10.08 编辑
  ...
  - 添加修补原厂 ``init_boot.img`` / ``boot.img`` 的方法
  - 添加新 LSPosed 的信息
  - 添加系统更新时保持 Root 的方法